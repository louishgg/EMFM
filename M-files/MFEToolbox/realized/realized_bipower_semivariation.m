function [bsvn,bsvp,bsvnDebiased,bsvpDebiased,bsvnSS,bsvpSS,bsvnSSDebiased,bsvpSSDebiased]=realized_bipower_semivariation(price,time,timeType,samplingType,samplingInterval,skip,subsamples)
% Computes bipower semivariation (BPSV), skip-k bipower semivariation and subsampled verisons of these
%
% USAGE:
%   [BSVN,BSVP,BSVND,BSVPD,BSVNSS,BSVPSS,BSVNSSD,BSVPSSD] = 
%           realized_bipower_semivariation(PRICE,TIME,TIMETYPE,SAMPLINGTYPE,SAMPLINGINTERVAL,SKIP,SUBSAMPLES)
%
% INPUTS:
...
rLag = returns(1:m-1-skip);
rLead = returns(2+skip:m);
bsvn = (pi/2) * abs(rLag)'*abs(rLead.*(rLead<0));
bsvp = (pi/2) * abs(rLag)'*abs(rLead.*(rLead>0));
biasScale = (m-1-skip) / m ;
bsvnDebiased = bsvn / biasScale;
bsvpDebiased = bsvp / biasScale;

subsampledLogPrices = realized_subsample(logPrice,time,timeType,samplingType,samplingInterval,subsamples);
bsvns = zeros(subsamples,1);
bsvps = zeros(subsamples,1);
totalCount  = 0;
for i=1:subsamples
    ...
    rLag = returns(1:m-1-skip);
    rLead = returns(2+skip:m);
    bsvns(i) = (pi/2) * abs(rLag)'*abs(rLead.*(rLead<0));
    bsvps(i) = (pi/2) * abs(rLag)'*abs(rLead.*(rLead>0));
    if i==1
        baseCount = m-1-skip;
    end
    totalCount = totalCount + m-1-skip;
end
bsvnSS = sum(bsvns) * (baseCount / totalCount);
bsvpSS = sum(bsvps) * (baseCount / totalCount);
bsvnSSDebiased = bsvnSS / biasScale;
bsvpSSDebiased = bsvpSS / biasScale;
